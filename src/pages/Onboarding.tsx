...
const timelineOptions = [{
  ...
  label: "1-3 meses",
  emoji: "⚡",
  description: "Metas rápidas e urgentes"
}, {
  value: "3-6",
  label: "3-6 meses",
  emoji: "📅",
  ...
  label: "6-12 meses",
  emoji: "📆",
  description: "Metas ambiciosas"
}, {
  value: "12+",
  label: "Mais de 1 ano",
  emoji: "🎯",
  description: "Grandes conquistas"
}, {
  value: "unsure",
  label: "Não sei ainda",
  emoji: "❓",
  description: "Vou decidir depois"
}];
const fadeInUp = {
  hidden: {
    opacity: 0,
...
const Onboarding = () => {
  ...
  return <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 flex items-center justify-center p-2 sm:p-4 relative overflow-hidden">
      {/* Back Button */}
      <button onClick={() => navigate("/")} className="absolute top-6 left-6 z-20 flex items-center gap-2 text-gray-600 hover:text-gray-900 transition-colors group">
      ...
      <div className="fixed bottom-0 left-0 right-0 h-64 z-0 pointer-events-none">
        <div className={cn("absolute bottom-0 left-0 right-0 h-full transition-all duration-700", currentStep === 0 && "track-blur-mentalidade", currentStep === 1 && "track-blur-organizacao", currentStep === 2 && "track-blur-aceleracao", currentStep === 3 && "track-blur-mentalidade", currentStep === 4 && "track-blur-organizacao", currentStep === 5 && "track-blur-aceleracao")} />
      </div>

      <div className="w-full max-w-lg mx-auto py-2 sm:py-4 relative z-10">
        {/* Logo */}
        <motion.div className="text-center mb-3 sm:mb-4" initial={{
        opacity: 0,
        y: -20
      }} animate={{
        opacity: 1,
        ...
        duration: 0.5
          ...
          <div className="flex justify-between mb-2">
            {steps.map((step, index) => <motion.div key={index} className="flex flex-col items-center" whileHover={{
            scale: 1.1
          }}>
                <motion.div className={cn("w-4 h-4 rounded-full cursor-pointer transition-colors duration-300", index < currentStep ? "bg-gradient-to-r from-indigo-600 to-purple-600" : index === currentStep ? "bg-gradient-to-r from-indigo-600 to-purple-600 ring-4 ring-indigo-600/20" : "bg-gray-300")} onClick={() => {
              ...
              scale: 0.95
                ...
                <motion.span className={cn("text-xs mt-1.5 hidden sm:block", index === currentStep ? "text-indigo-600 font-medium" : "text-gray-500")}>
                  {step.title}
                </motion.span>
              </motion.div>)}
          </div>
          <div className="w-full bg-gray-200 h-1.5 rounded-full overflow-hidden mt-2">
            <motion.div className="h-full bg-gradient-to-r from-indigo-600 to-purple-600" initial={{
            width: 0
          }} animate={{
          ...
          <Card className="border shadow-md rounded-3xl overflow-hidden bg-white max-h-[calc(100vh-8rem)] flex flex-col">
            <div className="flex-1 flex flex-col overflow-hidden">
              <AnimatePresence mode="wait">
                <motion.div key={currentStep} initial="hidden" animate="visible" exit="exit" variants={contentVariants} className="flex-1 overflow-y-auto px-1 sm:px-2">
                  {currentStep === 0 && <>
                      ...
                      <CardContent className="space-y-3">
                        <motion.div variants={fadeInUp} className="space-y-1.5">
                          ...
                          <Input id="name" placeholder="Digite seu nome" value={formData.name} onChange={e => updateField("name", e.target.value)} className="transition-all duration-300 focus:ring-2 focus:ring-purple-600/20 focus:border-purple-600" />
                        </motion.div>
                        <motion.div variants={fadeInUp} className="space-y-1.5">
                          <Label htmlFor="age">Quantos anos você tem?</Label>
                          <Input id="age" type="number" placeholder="Sua idade" value={formData.age} onChange={e => {
                        ...
                        <motion.div variants={fadeInUp} className="space-y-1.5">
                          ...
                          <Input id="password" type="password" placeholder="Pelo menos 6 caracteres" value={formData.password} onChange={e => updateField("password", e.target.value)} className="transition-all duration-300 focus:ring-2 focus:ring-purple-600/20 focus:border-purple-600" />
                        </motion.div>
                      </CardContent>
                    </>}
                  ...
                  {currentStep === 1 && <>
                      ...
                      <CardContent className="space-y-3">
                        <motion.div variants={fadeInUp} className="space-y-2">
                          <Input value={formData.goalName} onChange={e => updateField("goalName", e.target.value)} placeholder="ex: PlayStation 5, Notebook Novo, Ingresso de Show" className="text-lg p-5 transition-all duration-300 focus:ring-2 focus:ring-purple-600/20 focus:border-purple-600" />
                        </motion.div>
                      </CardContent>
                    </>}
                      ...
                      <CardContent className="space-y-3">
                        <motion.div variants={fadeInUp} className="space-y-2">
                          <div className="relative">
                            ...
                            <Input type="text" value={formatCurrency(formData.goalAmount)} onChange={e => {
                          const formatted = e.target.value;
                          const rawValue = parseCurrency(formatted);
                          updateField("goalAmount", rawValue);
                        }} placeholder="0" className="text-xl p-5 pl-11 transition-all duration-300 focus:ring-2 focus:ring-purple-600/20 focus:border-purple-600" />
                          </div>
                  ...
                  {currentStep === 3 && <>
                      ...
                      <CardContent className="space-y-3">
                        <motion.div variants={fadeInUp} className="space-y-2.5">
                          {timelineOptions.map(option => <motion.button key={option.value} onClick={() => updateField("goalTimeline", option.value)} className={cn("w-full p-3 rounded-xl border-2 transition-all text-left", formData.goalTimeline === option.value ? "border-purple-600 bg-purple-50" : "border-gray-200 hover:bg-gray-50")} whileHover={{
                        scale: 1.02
                      }} whileTap={{
                        scale: 0.98
                      }}>
                  ...
                  {currentStep === 4 && <>
                      ...
                      <CardContent className="space-y-3">
                        <motion.div variants={fadeInUp} className="grid grid-cols-2 gap-3">
                          <motion.button onClick={() => updateField("incomeType", "mesada")} className={cn("p-4 rounded-2xl border-2 transition-all", formData.incomeType === "mesada" ? "border-purple-600 bg-purple-50" : "border-gray-200 hover:bg-gray-50")} whileHover={{
                        scale: 1.02
                      }} whileTap={{
                        scale: 0.98
                      }}>
                  ...
                  {currentStep === 5 && <>
                      ...
                      <CardContent className="space-y-2">
                        <AvatarPicker selectedAvatarId={formData.avatarId} onAvatarChange={id => updateField("avatarId", id)} />
                      </CardContent>
                    </>}
              ...
              <CardFooter className="flex justify-between pt-4 pb-3">
                ...
                scale: 0.95
                  ...
                  <Button type="button" variant="outline" onClick={prevStep} disabled={loading || currentStep === 0} className="flex items-center gap-1 transition-all duration-300 rounded-2xl">
                ...
                scale: 0.95
                  ...
                  <Button type="button" onClick={currentStep === steps.length - 1 ? handleSubmit : nextStep} disabled={!isStepValid() || loading} className="flex items-center gap-2 transition-all duration-300 h-12 rounded-xl bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white font-semibold shadow-lg hover:shadow-xl hover:shadow-indigo-200 active:scale-[0.98] px-6">
                    {loading ? <>
