...
interface CreateRoomDialogProps {
  ...
  onRoomCreated: (matchId: string) => void;
  userId: string;
}
  ...
  const handleCreate = async () => {
    ...
    if (currentXp < 10) {
      toast({
        ...
        description: "Você precisa de pelo menos 10 XP para criar uma sala.",
        variant: "destructive",
      });
      return;
    }
    ...
    if (xpBet > currentXp) {
      toast({ 
        title: "XP insuficiente 🎮", 
        description: "Você precisa de mais XP! Complete módulos na aba Mentalidade para ganhar XP.",
        variant: "destructive",
        duration: 6000
      });
      return;
    ...
    try {
      ...
      let errorMessage = "Não foi possível criar a sala. Tente novamente.";
      
      if (error.message?.includes("permission")) {
        errorMessage = "Você não tem permissão para criar salas.";
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="bg-gradient-to-br from-purple-900/95 to-pink-900/95 border-purple-500 text-white">
        ...
        <div className="space-y-4">
          ...
          <div>
            <Label className="text-purple-200">Aposta de XP (por jogador)</Label>
            <Slider
              value={[xpBet]}
