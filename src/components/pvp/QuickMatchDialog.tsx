...
export const QuickMatchDialog = ({
  ...
  useEffect(() => {
    if (!queueId) return;
    const channel = supabase.channel(`queue-${queueId}`).on('postgres_changes', {
      event: 'UPDATE',
      schema: 'public',
      table: 'pvp_queue',
      ...
      if (updated.status === 'matched' && updated.match_id) {
        ...
        const {
          data: match
        } = await supabase.from("pvp_matches").select("*").eq("id", updated.match_id).single();
        if (match) {
          setSearching(false);
          setQueueId(null);
          toast({
  ...
  const loadUserXp = async () => {
    const {
      data
    } = await supabase.from("profiles").select("current_xp").eq("id", userId).single();
  ...
  const handleCancel = async () => {
    if (queueId) {
      ...
      // Cancel queue entry
      await supabase.from("pvp_queue").update({
        status: "cancelled"
      }).eq("id", queueId);
    }
    setSearching(false);
  ...
  return <Dialog open={open} onOpenChange={searching ? () => {} : onOpenChange}>
      <DialogContent className="bg-gradient-to-br from-purple-900/95 to-pink-900/95 border-purple-500 text-white">
        ...
        {!searching ? <div className="space-y-4">
            ...
            <div>
              ...
              <Slider value={[xpBet]} onValueChange={([value]) => setXpBet(value)} min={10} max={Math.max(10, Math.min(500, userXp))} step={10} className="mb-2" />
              ...
              {xpBet > userXp && <p className="text-xs text-red-400 mt-2">
